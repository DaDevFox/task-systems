...
func TestTaskService(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			service := NewTaskService(repo, 2) // Small inbox for testing
			tt.test(t, service)
		})
	}
}
...
func testAddTask(t *testing.T, service *TaskService) {
	...
	task, err := service.AddTask(ctx, "Test Task", "Test Description")
	if err != nil {
	...
	if task.Stage != domain.StagePending {
		t.Errorf("Expected stage pending, got %s", task.Stage)
	}
...
func testAddTaskEmptyName(t *testing.T, service *TaskService) {
	...
	_, err := service.AddTask(ctx, "", "Test Description")
	if err == nil {
...
func testMoveToStagingWithDestination(t *testing.T, service *TaskService) {
	...
	destTask, err := service.AddTask(ctx, "Dest Task", "Destination")
	if err != nil {
		t.Fatalf("AddTask failed: %v", err)
	}
	...
	// Create source task
	sourceTask, err := service.AddTask(ctx, "Source Task", "Source")
	if err != nil {
		t.Fatalf("AddTask failed: %v", err)
	}
...
func testStartTaskWithDependencies(t *testing.T, service *TaskService) {
	...
	depTask, err := service.AddTask(ctx, "Dependency", "Dependency task")
	if err != nil {
		t.Fatalf("AddTask failed: %v", err)
	}
...
func testMergeTasks(t *testing.T, service *TaskService) {
	...
	task1, err := service.AddTask(ctx, "Task 1", "Description 1")
	if err != nil {
		t.Fatalf("AddTask 1 failed: %v", err)
	}

	task2, err := service.AddTask(ctx, "Task 2", "Description 2")
	if err != nil {
...
func testSplitTask(t *testing.T, service *TaskService) {
	...
	// Create a task
	task, err := service.AddTask(ctx, "Original Task", "Original Description")
	if err != nil {
		t.Fatalf("AddTask failed: %v", err)
	}
...
func testAdvertiseTask(t *testing.T, service *TaskService) {
	...
	// Create source task
	sourceTask, err := service.AddTask(ctx, "Source", "Source task")
	if err != nil {
		t.Fatalf("AddTask source failed: %v", err)
	}

	// Create target tasks
	target1, err := service.AddTask(ctx, "Target 1", "Target 1")
	if err != nil {
		t.Fatalf("AddTask target1 failed: %v", err)
	}

	target2, err := service.AddTask(ctx, "Target 2", "Target 2")
	if err != nil {
...
func testStitchTasks(t *testing.T, service *TaskService) {
	...
	// Create source tasks
	source1, err := service.AddTask(ctx, "Source 1", "Source 1")
	if err != nil {
		t.Fatalf("AddTask source1 failed: %v", err)
	}

	source2, err := service.AddTask(ctx, "Source 2", "Source 2")
	if err != nil {
		t.Fatalf("AddTask source2 failed: %v", err)
	}

	// Create target task
	target, err := service.AddTask(ctx, "Target", "Target task")
	if err != nil {
...
func testInboxConstraint(t *testing.T, service *TaskService) {
	...
	task3, err := service.AddTask(ctx, "Task 3", "Description 3")
	if err != nil {
