...
// These tests use separate environments for each test function to avoid race conditions
// while still testing the same functionality as the intermingled version

func TestServiceAPIInitialHistoryCreated_Isolated(t *testing.T) {
	// Each test gets its own service instance and database
...
func TestServiceAPIUpdateInventoryLevelCreatesHistory_Isolated(t *testing.T) {
	// Each test gets its own service instance and database
	service, cleanup := setupTestServiceWithRealDB(t)
	defer cleanup()
...
func TestServiceAPIMultipleUpdatesCreateTimeline_Isolated(t *testing.T) {
	// Each test gets its own service instance and database
	service, cleanup := setupTestServiceWithRealDB(t)
	defer cleanup()
...
func TestServiceAPITimeRangeQuery_Isolated(t *testing.T) {
	// Each test gets its own service instance and database
	service, cleanup := setupTestServiceWithRealDB(t)
	defer cleanup()
