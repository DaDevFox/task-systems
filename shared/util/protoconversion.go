...
func ValidEventType(event *pb.Event) bool {
	return GetEventType(event) == event.Type // Fixed: should be == not !=
}
...
// ProtoToMap converts protobuf to map (placeholder implementation)
func ProtoToMap(event *pb.Event) map[string]interface{} {
	// This is a placeholder - in a real implementation you'd use protobuf reflection
